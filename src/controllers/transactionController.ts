...
export const send = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    const { sender_account_number, receiver_account_number, amount, currency } = request.body as TransactionRequest;
...
export const withdraw = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    const { sender_account_number, amount, currency } = request.body as TransactionRequest;
    const user_id = request.user.id;

    const transaction = await handleTransaction(user_id, sender_account_number, null, amount, currency, TRANSACTION_TYPE.WITHDRAW);
    reply.send(transaction);
  } catch (error: any) {
    reply.status(400).send({ error: error.message });
  }
